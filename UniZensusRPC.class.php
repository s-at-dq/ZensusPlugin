...
class UniZensusRPC {
    ...
    function getCourseStatus($course_id, $user_id = null){
        ...
        $hash = hash_hmac(
            'md5',
            $course_id.$tstamp.$user_id,
            Config::get()->UNIZENSUSPLUGIN_SHARED_SECRET1 . Config::get()->UNIZENSUSPLUGIN_SHARED_SECRET2
        );
        if (is_null($user_id)){
            $msg = new xmlrpcmsg(
                "info.course_status_v3",
                array(
                    new xmlrpcval($course_id, "string"),
